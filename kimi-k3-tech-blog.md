...
- **2.8 万亿总参数、激活参数 104B**（官方模型卡数据），全球首个达到 3T 级别的开源权重模型；此前最大开源模型为 1.6T 的 DeepSeek V4 Pro（第三方报道口径，非官方模型卡内容）。
...
| 模态 | 文本、图像、视频 → 文本（原生多模态，非拼接式；模型卡 Modality 字段标为 "Text, Image"，视频输入见模型卡介绍与 Quickstart 视觉文档） |
...
- 官方口径：整体扩展效率较 Kimi K2 提升约 **2.5×**（官方技术博客）；AttnRes 技术报告称其训练开销为"边际化"（marginal training overhead）、推理开销"极小"（minimal inference overhead），约 **8 个块**可恢复 Full AttnRes 大部分收益（arXiv:2603.15031）。
- ⚠️ **2026-08-02 核验修正**：早期版本此处引用"不到 2% 的额外开销、约 25% 的训练效率提升"并标为官方数据；经与官方技术博客、AttnRes 论文原文、官方模型卡逐一比对，**均未公布这两个具体百分比**，出处未能确认，故撤回"官方数据"标注。如需引用该组数字，请先自行溯源。
...
| DECK-Bench | 73.5 | ⚠️ 官方模型卡完整表未收录此基准，来源待确认（疑似第三方聚合评测） |
