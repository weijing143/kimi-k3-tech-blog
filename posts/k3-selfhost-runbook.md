...
## 8. 第三方发布前估算汇总（⚠️ 未经实测验证）

> **阅读警告**：本节汇总 2026-07-17 ~ 07-22 期间公开发布的第三方部署指南中的数字。这些文章**全部早于权重发布日（2026-07-27）**，因此其中的"实测"声明不可能是对公开权重的真机实测，本质是**发布前预测/推算**。收录目的是给规划阶段的读者一个交叉参照，**不作为实测数据**，请勿引用为"K3 实测结果"。检索日期：2026-07-28。

### 8.1 口径一致、可参考的估算

| 结论 | 内容 | 来源（发布日期） |
| --- | --- | --- |
| 4-bit 权重体积 | MXFP4 / AWQ 4-bit 约 **1.4 TB**，与 2.8T × 0.5 B 算术吻合 | namoclaw.ai（2026-07-20） |
| KV cache 随上下文线性增长 | 减半上下文 = 减半 KV cache；显存预算 = 权重 + KV + 激活 + 运行时开销 | wan27.org（2026-07-21） |
| 最低实验性配置 | 量化 + 缩短上下文（128K–256K）后约 **4–6× H100 80G** 可加载；生产需更多 | wan27.org / ai-indeed.com / treerouter.ai（2026-07-20~22） |
| 单卡不可行 | 任何精度下权重视都超过单卡 HBM（H200 141 GB） | wan27.org（2026-07-21） |
| 部署参数共识 | `tensor-parallel-size 8` + `enable-expert-parallel` + `max-model-len` 按需 + `trust-remote-code` | CSDN / treerouter.ai（2026-07-22） |
| 运维门槛 | 自部署需推理框架维护、监控告警、扩缩容，约需 1 名专职工程师 | wan27.org 中文站（2026-07-17） |

### 8.2 存在硬伤、**不要引用**的说法

| 说法 | 出处 | 问题 |
| --- | --- | --- |
| "BF16 权重 ~594 GB" | wan27.org（2026-07-21） | 2.8T 参数 BF16 应约 5.6 TB，594 GB 对不上任何标准精度，疑似从 K2.6（约 1.1T 参数，Int4 约 595 GB）抄错 |
| "8× H100 实测吞吐 ~2200 tok/s（batch 32）" | namoclaw.ai（2026-07-20） | 发布早于权重公开 7 天，不可能实测；同文还把 License 误写为 Apache-2.0 |
| "K3 采用 MIT / Apache-2.0 许可" | 多篇文章 | 实际是自定义 **Kimi K3 License**（本仓库主文档有条款拆解），引用前务必以 HF 模型卡为准 |
| "部署测试基于真实 OOM 边界测量" | wan27.org（2026-07-21） | 发布早于权重公开 6 天，"实测"声明存疑，其逐精度显存表可当估算参考但不能当测量值 |

### 8.3 使用建议

1. 规划硬件预算时，**优先用 §1 的算术自行复算**，第三方表格只作交叉验证；
2. 任何标注"实测"但发布日期早于 2026-07-27 的数据，一律视为预测口径；
3. 权重发布后社区的真机实测（TTFT / 吞吐 / 显存）预计陆续放出，发现可信来源欢迎按 [CONTRIBUTING](../CONTRIBUTING.md) 的数据规范提交 PR，本节将持续更新。
