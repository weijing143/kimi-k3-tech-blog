...
1. **外部权威 / 社区实测**：用于提供真实环境参照，必须能追溯到测试环境、方法和原始证据；收录不代表本项目复现；
2. **本项目复现实测**：使用本节脚本或等价公开方法实际执行，保留完整命令与原始输出后，才能填入 §5 的结果表。

外部实测统一登记在下表，不直接抄入 §5 的本项目结果表。来源若缺少模型版本、硬件、引擎版本、负载定义或原始结果，只能作为线索记录在 issue 中，不能进入本表。

| ID | 来源 / 测试日期 | 模型版本或哈希 | 硬件 | 引擎 / 版本 / 启动参数 | 对应实验 | 原始证据 | 复现状态 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| EXT-001 | 待收录 | — | — | — | — | — | 外部实测，项目未复现 |

每条记录还必须在原始证据或配套说明中提供：TP/EP/PP、输入和输出长度、并发数、请求数、错误率、预热方法以及指标定义。外部来源的许可证或引用条件不明确时，只登记链接与事实摘要，不复制其原始文件。

## 5. 实验设计与结果表（本项目复现，留空待填）

以下表格只填写本项目实际执行得到的结果。复现外部数据时，须在环境行注明对应的 `EXT-*` ID、条件差异与原始 JSON 路径；仅引用外部数字时保持本表留空。
...
环境：GPU ___｜引擎 ___ 版本 ___｜启动参数 ___｜日期 ___｜对照外部记录（可选）EXT-___｜原始结果 ___
...
环境：GPU ___｜启动参数 ___｜日期 ___｜对照外部记录（可选）EXT-___｜原始结果 ___
...
评测集/版本 ___｜评分规则 ___｜推理参数 ___｜日期 ___｜对照外部记录（可选）EXT-___｜原始结果 ___
...
>
> 权重发布后的可信外部真机数据应登记到 §4.1 的外部实测索引；本节只保留历史估算，不随外部实测更新而改写其性质。
